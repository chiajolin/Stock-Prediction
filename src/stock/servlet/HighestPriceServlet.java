 ...
 */
@WebServlet("/HighestPriceServlet")
public class HighestPriceServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HighestPriceServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {  
        	...
        	String sql = "SELECT D.Stock_Id, D.Name, Max(D.Close) AS Price "+
        				 "FROM ( "+
        				 "SELECT H.Stock_Id, H.Close, H.Time, S.Name "+
        				"FROM Historical H, Stock S WHERE S.Id = H.Stock_Id and S.Id IN (" + IdSet + ") "+
        				 "ORDER BY H.Time DESC LIMIT 10) D GROUP BY D.Stock_Id";
